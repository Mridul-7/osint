...
message = """
  ...
  _____    __  __   _____     ____   _____ _____ _   _ _______ 
 |  __ \  |  \/  | |  __ \   / __ \ / ____|_   _| \ | |__   __|
 | |__) | | \  / | | |__) | | |  | | (___   | | |  \| |  | |   
 |  ___/  | |\/| | |  _  /  | |  | |\___ \  | | | . ` |  | |   
 | |      | |  | | | | \ \  | |__| |____) |_| |_| |\  |  | |   
 |_|      |_|  |_| |_|  \_\  \____/|_____/|_____|_| \_|  |_|   
...
def print_short(res):
    ...
    loc = "%s / %s" % (
        res.get("location.country_code", "N/A"),
        res.get("location.city", "N/A"),
    )
...
def CensysPublicReport(engine, public_info, output_basename):
    ...
    if output_basename:
        ...
        with open(output1, "a") as txt, open(output2, "a") as md, open(
            output3, "a"
        ) as xml, open(output4, "a") as html:
            txt.write("[+] {} results\n".format(engine))
            txt.write("-------------------------\n")
            md.write("---\n\n")
            md.write("## {} results\n".format(engine))
...
def CensysPublicScan(
    ...
    if args.mode == "censys" and (
        ...
        or args.report != None
        or args.html != False
        or args.http_server != None
        or args.tags != None
        or args.cert_org != None
        or args.cert_host != None
        or args.count != False
        or args.html_body != None
        or args.html_title != None
        or args.country != None
    ):
...
def build_query_string(args):
    ...
    if args.cert_org:
        s += (
            " AND 443.https.tls.certificate.parsed.subject.organization:%s"
            % args.cert_org
        )
    if args.cert_issuer:
        s += (
            " AND 443.https.tls.certificate.parsed.issuer.organization:%s"
            % args.cert_issuer
        )
    if args.cert_host:
        s += (
            " AND 443.https.tls.certificate.parsed.subject.common_name:%s"
            % args.cert_host
        )
    if args.country:
...
def print_res(path, match, val):
    ...
    if len(match) >= 80:
        ...
        match_c = (
            ...
            + Fore.RED
            + Style.BRIGHT
            + match[pos : pos + len(val)]
            + Style.RESET_ALL
            + Style.DIM
            + match[pos + len(val) : pos + 35]
            + post
            + Style.RESET_ALL
        )
...
def CheckDomainOrIP(value):
    ...
    if not validators.domain(value) and not validators.ip_address.ipv4(value):
        raise argparse.ArgumentTypeError(
            "Invalid domain or ip address ({}).".format(value)
        )
    return value
...
def GetEmails(results, value):
    ...
    temp = re.compile("[a-zA-Z0-9.\-_+#~!$&',;=:]+" + "@" + "[a-zA-Z0-9.-]*" + value)
    ...
    while counter <= limit:
        try:
            ...
            r = s.get(
                url,
                verify=False,
                headers={"User-Agent": PickRandomUA(uas)},
                proxies=proxies,
            )
            if r.status_code != 200:
                print(
                    "[-] Something is going wrong (status code: {})".format(
                        r.status_code
                    )
                )
                return [], []
...
def CrtSearch(value, uas, proxies):
    ...
    try:
        ...
        if r.status_code != 200:
            print(
                "[-] Something is going wrong (status code: {})".format(r.status_code)
            )
            return [], []
        results += r.text
    except Exception as e:
...
def DNSDumpsterSearch(targetip, uas, proxies):
    ...
    try:
        ...
        s = requests.Session()
        myheaders = {
            "User-Agent": PickRandomUA(uas),
            "Referer": "https://dnsdumpster.com",
        }
        r = s.get(
            url, verify=False, headers=myheaders, proxies=proxies, timeout=timeout
        )
        if r.status_code != 200:
            print(
                "[-] Something is going wrong (status code: {})".format(r.status_code)
        ...
        if pr.status_code != 200:
            print(
                "[-] Something is going wrong (status code: {})".format(pr.status_code)
            )
            return [], []
...
def InfoReport(mode, limit, dnsserver, proxy, domain, ip, uas, output_basename):
    ...
    if output_basename:
        ...
        with open(output1, "w") as txt, open(output2, "w") as md, open(
            output3, "w"
        ) as xml, open(output4, "w") as html:
            txt.write("{}\n".format(message))
            md.write("```\n")
            md.write("{}\n".format(message))
            md.write("```\n\n")
            ...
            html.write(
                "<!DOCTYPE html><html><head><title>GasMasK Report</title></head><body>\n"
            )
            html.write("<pre>{}</pre>\n</br>\n<ul>\n".format(message))
            ...
            html.write(
                "<li>Information gathering: <b>{}</b></li>\n".format(",".join(mode))
            )
            ...
            for l in lines:
                if l.lower().startswith(service_name) and ":" in l:
                    try:
                        ...
                        print(
                            KEYS_FILE + " doesnt follow the correct format name:value"
                        )
...
def MainFunc():
    ...
    report_buckets = 50
    info = {"all_emails": [], "all_hosts": [], "domains": [], "public": []}
    uas = []
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Use a proxy server when retrieving "
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output in the four major formats at once "
    ...
    parser.add_argument(
        "--count", action="store_true", help="Print the count result and exit"
    )
    parser.add_argument(
        "-R",
        "--report",
    ...
    parser.add_argument(
        "-L", "--Limit", default=float("inf"), help="Limit to N results"
    )
    parser.add_argument("-D", "--debug", action="store_true", help="Debug information")
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Print raw JSON records"
    )
    parser.add_argument(
        "-H", "--html", action="store_true", help="Renders html elements in a browser"
    )
    parser.add_argument(
        "arguments", metavar="arguments", nargs="*", help="Censys query"
    )
    ...
    if any(i in ["vhosts", "basic", "nongoogle"] for i in info["mode"]):
        ...
        print("----------------------")
        info["bingvhosts"] = BingVHostsSearch(
            info["ip"], info["limit"], uas, info["proxies"], timeouts
        )
    ...
    if any(i in ["google"] for i in info["mode"]):
        ...
        temp1, temp2 = GoogleSearch(
            info["domain"], info["limit"], uas, info["proxies"], timeouts
        )
    ...
    if any(i in ["shodan"] for i in info["mode"]):
        ...
        if args.shodankey is None:
            print(
                "[-] API key required for the Shodan search: '-k API-KEY, --shodan-key API-KEY'"
            )
            sys.exit()
        ...
        results = ShodanSearch(
            args.shodankey, info["domain"], info["ip"], uas, info["proxies"], timeouts
        )
    ...
    if any(i in ["ask", "nongoogle"] for i in info["mode"]):
        ...
        temp1, temp2 = AskSearch(
            info["domain"], 5, uas, info["proxies"], timeouts
        )  # 5 pages
        info["all_emails"].extend(temp1)
        info["all_hosts"].extend(temp2)
    ...
    if any(i in ["spyse", "nongoogle"] for i in info["mode"]):
        ...
        if not spysekey:
            print(
                "Api Key for spyse was neither provided in cmd line "
    ...
    if any(i in ["linkedin"] for i in info["mode"]):
        ...
        temp1, temp2 = GoogleSearchEngine(
            info["domain"],
            "linkedin.com",
            info["limit"],
            uas,
    ...
    if any(i in ["twitter"] for i in info["mode"]):
        ...
        temp1, temp2 = GoogleSearchEngine(
            info["domain"], "twitter.com", info["limit"], uas, info["proxies"], timeouts
        )
        info["all_emails"].extend(temp1)
        info["all_hosts"].extend(temp2)
    ...
    if any(i in ["youtube"] for i in info["mode"]):
        ...
        temp1, temp2 = GoogleSearchEngine(
            info["domain"], "youtube.com", info["limit"], uas, info["proxies"], timeouts
        )
        info["all_emails"].extend(temp1)
        info["all_hosts"].extend(temp2)
    ...
    if any(i in ["reddit"] for i in info["mode"]):
        ...
        temp1, temp2 = GoogleSearchEngine(
            info["domain"], "reddit.com", info["limit"], uas, info["proxies"], timeouts
        )
        info["all_emails"].extend(temp1)
        info["all_hosts"].extend(temp2)
    ...
    if any(i in ["github"] for i in info["mode"]):
        ...
        temp1, temp2 = GoogleSearchEngine(
            info["domain"], "github.com", info["limit"], uas, info["proxies"], timeouts
        )
        info["all_emails"].extend(temp1)
        info["all_hosts"].extend(temp2)
    ...
    if any(i in ["censys"] for i in info["mode"]):
        ...
        if api_id is not None and api_secret is not None:
            ...
            res1 = DomainSearchCensys(
                info["domain"], api_id, api_secret, output_basename, info["domains"]
            )
                        ...
                        if answer1 == "y":
                            with open(KEYS_FILE) as f:
                                ...
                                for line in lines:
                                    ...
                                    if res1 == False and res2 == False:
                                        print(
                                            "Please use the available censys.io options in order"
